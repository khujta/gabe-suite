---
name: gabe-handoff
description: "Session handoff — emit a paste-able next-session prompt + sync durable KDBP state so a fresh session resumes with full fidelity. Run when context is heavy or you're wrapping up. Usage: /gabe-handoff [--dry-run | --no-sync | <focus note>]"
---
