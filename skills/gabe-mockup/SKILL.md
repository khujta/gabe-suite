...
2. **RF2 — Locate or scaffold the layout-policy oracle.** Read the project's layout policy doc (e.g. `docs/rebuild/WEB-LAYOUT-POLICY.md`). If none exists, scaffold one from the policy below and record a KDBP decision. The policy (P1–P6) is the machine-checked oracle:
   ...
   - **P7 Full-surface sheets/overlays** (add/edit/create flows that cover the frame): in live the sheet covers the app frame EXACTLY (`inset-0`, no framed-device negative-inset bleed that pushes the footer off-screen); the close button matches the app-header profile-button footprint (same size + position); the title is vertically centered with breathing room and its icon/glyph is NOT inside a `truncate`/overflow-hidden box (truncate the text span only — otherwise the glyph's border/shadow is clipped); the footer is a FLOATING action bar — pinned to the bottom with a transparent gradient backdrop so content scrolls visibly behind it, honoring `env(safe-area-inset-bottom)` so CTAs clear the system nav bar, with matching bottom padding on the scroll body.
...
5. **RF5 — Fix: apply the live-mode layout recipe.** For each failing policy, apply the proven recipe. Gate EVERY change on a `live`/`chromeless` (or equivalent) flag that defaults OFF so the Storybook stories stay byte-identical:
   ...
   - **Full-surface sheets/overlays** — in live cover the frame exactly (`inset-0`, drop framed-device negative insets); close button = app-header profile-button footprint; title centered + breathing room, glyph NOT in a `truncate` box; footer = FLOATING action bar (`absolute inset-x-0 bottom-0` + `bg-gradient-to-t from-bg via-bg/92 to-transparent`, no solid block) honoring `env(safe-area-inset-bottom)`, with matching `pb` on the scroll body so the last item clears it.
