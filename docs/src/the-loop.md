...
A session should finish one phase, or leave it in a state the next session can resume cleanly. That means: don't stop mid-task with uncommitted work and no note about what's left; don't let Exec sit at 🔄 with a stale task list nobody will recognize a week from now. Every phase transition is a natural place to end a session — the four-column tick is already the checkpoint, so honoring it costs nothing extra. When you *do* have to stop mid-task, `/gabe-handoff` writes that resume state for you — a paste-able next-session prompt plus a synced `.kdbp/` (see [Command reference](commands.html) § session continuity) — so the interruption lands on a clean boundary even when the tick alone wouldn't capture it.
:::
